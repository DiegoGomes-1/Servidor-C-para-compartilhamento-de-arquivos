...
int main()
{
  ...
  {	
    ...
    if(strstr(buff,"/ ")!=NULL)
    {
        ...
        for(i=2;i<qntArq;i++){
           char aux[1000];
           sprintf(aux,"<a href=\"http://localhost:8080/nuvem/%s\">%s</a>\n<br>",namelist[i]->d_name,namelist[i]->d_name);
           strcat(response,aux);
       ...
       *q = '\0';
       ...
       sprintf(caminho,"/home/diego/Área de Trabalho/tp1_Repositorio/nuvem/%s",p); //Altere para seu diretorio pessoal  
       printf("%s\n",caminho);
       char *r = strstr(p,".");
       r++;
      ...
      bzero(buff,sizeof(buff));
        ...
        FILE *arq = fopen("/home/diego/Área de Trabalho/tp1_Repositorio/info.txt","rb+"); //Altere para seu diretorio pessoal
        ...
        FILE *arq = fopen("/home/diego/Área de Trabalho/tp1_Repositorio/hello.html","rb"); //Altere para seu diretorio pessoal
        char header[10000];
        if(arq == NULL){ printf ("arquivo não abriu\n");}
        else
        {
